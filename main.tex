...
%%% Question: "a list whose length grows with respect to the number of
%%% binders" Can this be rewritten to "a list with one entry for each
%%% binder that is in scope"? That is, is the length of the list equal
%%% to the binders, or just O(n) where n is the number of binders?
...
%%% This URL will not be discoverable if someone prints the
%%% paper. What about putting it in a footnote instead?

\section{De Bruijn numbers should coexist with names}
\label{closures}

%%% General comment: de Bruijn numbers can be either de Bruijn indices
%%% or de Bruijn levels. Here, we are using indices, but using the
%%% less-precise term "numbers" consistently. I had suggested writing
%%% "indices" in a previous version, which Weixi was against, but I
%%% don't remember why, so I'm not changing it here. But it's worth
%%% looking at.
...
%%% We should explicitly mention the parts of the problem that we
%%% don't solve here.
...
%%% What does "superimposing" mean here?
