...
given an index we can find its atom, if no shadowings occur.
Figure~\ref{table:same-atom} defines the rules to decide whether two
atoms are $\alpha$-equivalent w.r.t their scopes, written as
...
\begin{definition}
  ...
  For the simplified problems, we restrict $t$ to an atom.
\end{definition}
...
we refer to an atom-atom or atom-var equation as
\emph{$e_\nu$} and refer to a
...
extend a scope with an atom, we extend the two hashtables with the
...
where $n$ is the sum of atom and variable occurrences.
...
the update operation of HAMT and the generation of atoms and
